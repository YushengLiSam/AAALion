...
### 10 — 效果数据 / Eval
- **recall@5 0.947** (92-case canonical) / **0.832** (61-case compositional —
  multi-turn / comparison), **MRR 0.86**, **negation accuracy 1.000 on both**.
  Cross-encoder rerank lifts recall **0.74→0.95** (92) / **0.58→0.83** (61) —
  the headline ablation. recall@5 is *fractional* (|hit|/|expected|); the
  61-case is comparison-heavy, so binary "shows ≥1 relevant" is **0.983** live —
  the user almost always sees a right product. 145-product catalog; eval dashboard.
  *(current numbers 2026-06-10, post the negation + demo-hardening fixes; the
  old 0.88/0.83 in earlier decks is stale — use these.)*
...
- Hybrid retrieval cache (cold ~15-18s → **warm ~1s**, measured), two cache
  ...
  to a GCP VM (~2 min, rollback on failure); server tests + **92+61-case** eval.
- *Rubric: 工程质量.*
...
- **"鉴权/会话安全?"** → login issues a **signed, expiring HS256 JWT**
  (`POST /auth/verify` validates it; tampered/expired tokens rejected). The
  opaque `token == user_id` is a demo convenience on top; passwords are
